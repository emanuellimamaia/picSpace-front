...
export default function SingUpForm() {
  ...
  async function handleSingUp(form: SingUpFormSchema) {
    try {
      ...
      const { accessToken, name, email, role, username } = response.data;
      ...
      setCookie(undefined, "fulog.token", accessToken, {
        maxAge: 60 * 60 * 24 * 30,
        path: "/",
      });
      ...
      setUserData({ username: name, token: accessToken, role: role });
  ...
  return (
    <div className="w-full">
      <h2 className="text-2xl font-semibold text-[#64ffda] mb-6">Criar Conta</h2>
      <form
        className="flex flex-col gap-6"
        onSubmit={handleSubmit(handleSingUp)}
      >
        <div className="space-y-2">
          <label className="text-sm font-medium text-gray-300">Nome do usuário</label>
          <Input
            type="text"
